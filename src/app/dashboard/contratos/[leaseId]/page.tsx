...
export default async function ContratoPage({ params }: { params: Promise<{ leaseId: string }> }) {
  ...
  const { data: lease } = await supabase
    .from('leases')
    .select('id, rent_value, start_date, end_date, due_day, adjustment_index, adjustment_period_months, property_id, tenant_id, landlord_profile_id')
    .eq('id', leaseId)
    .single()
  ...
  const property = propertyRaw as { name: string; address: string | null; city: string | null; state: string | null } | null
  const tenant = tenantRaw as { name: string; document: string | null; email: string | null; phone: string | null; street: string | null; street_number: string | null; district: string | null; city: string | null; state: string | null } | null
    ...
    <>
      ...
      <div className="contract-box" style={{
        ...
        <section style={{ marginBottom: '28px' }}>
          <h2 style={{ fontSize: '13px', textTransform: 'uppercase', letterSpacing: '2px', color: '#888', margin: '0 0 14px', borderBottom: '1px solid #e8e8e8', paddingBottom: '6px' }}>
        ...
        <section style={{ marginBottom: '28px' }}>
          ...
          <p style={{ fontSize: '14px', margin: '0 0 6px' }}><strong>Do Locatário:</strong></p>
          ...
          <p style={{ fontSize: '14px', margin: '0 0 6px' }}><strong>Do Locador:</strong></p>
        ...
        {/* Assinaturas */}
        <div style={{ borderTop: '1px solid #e0e0e0', paddingTop: '24px', marginBottom: '16px' }}>
