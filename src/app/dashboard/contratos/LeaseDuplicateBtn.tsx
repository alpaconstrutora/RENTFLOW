...
import { createLeaseAction, runBackfillAction, getLeaseDiscountsAction } from './actions'
...
export default function LeaseDuplicateBtn({ lease, properties, tenants, landlordProfiles }: Props) {
  ...
  const [dueDay, setDueDay] = useState(lease.due_day)

  useEffect(() => {
    if (isOpen) {
      setDueDay(lease.due_day)
      getLeaseDiscountsAction(lease.id).then(d => {
        setDiscounts(d.map(item => ({
          start_installment: item.start_installment,
          end_installment: item.end_installment,
          discount_value: item.discount_value
        })))
      })
    }
  }, [isOpen, lease.id, lease.due_day])
  ...
  return (
    <>
      ...
      {isOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(5, 5, 8, 0.85)', backdropFilter: 'blur(20px)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '40px 20px', overflowY: 'auto', textAlign: 'left' }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '640px', backgroundColor: 'rgba(25, 28, 38, 0.95)', padding: '32px 24px', position: 'relative', borderRadius: '24px', boxShadow: '0 30px 60px rgba(0,0,0,1)', border: '1px solid rgba(255,255,255,0.1)', boxSizing: 'border-box', marginBottom: '40px' }}>
            ...
            {backfillInfo && !backfillDone ? (
              ...
              <>
                <h2 style={{ fontSize: '26px', fontFamily: 'var(--font-heading)', marginBottom: '8px', color: 'white' }}>Duplicar Contrato (Nova Cópia)</h2>
                <p style={{ color: 'var(--text-secondary)', marginBottom: '28px', fontSize: '14px' }}>Os dados abaixo foram copiados do contrato original. Revise os campos e efetive para criar o novo contrato.</p>
                ...
                <form action={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <select name="property_id" required style={inputStyle} defaultValue={lease.property_id ?? ''}>
                        <option value="">-- Selecione o Imóvel --</option>
                        {properties.map(p => (
                          <option key={p.id} value={p.id}>
                            {p.name}{p.status !== 'vacant' && p.id !== lease.property_id ? ' (Ocupado)' : ''}
                          </option>
                        ))}
                      </select>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <select name="tenant_id" required style={inputStyle} defaultValue={lease.tenant_id ?? ''}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <input name="rent_value" type="number" step="0.01" required defaultValue={lease.rent_value} style={inputStyle} />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <input
                        name="due_day" type="number" min="1" max="31" required
                        value={dueDay}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <input name="start_date" type="date" required defaultValue={lease.start_date?.split('T')[0] ?? ''} style={inputStyle} />
                    </div>
                  </div>
                  ...
                  {landlordProfiles.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <select name="landlord_profile_id" style={inputStyle} defaultValue={lease.landlord_profile_id ?? ''}>
                  ...
                  <div style={{ background: 'rgba(99,102,241,0.05)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '14px', padding: '16px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <select name="adjustment_period_months" style={inputStyle} defaultValue={lease.adjustment_period_months ?? '12'}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <select name="adjustment_index" style={inputStyle} defaultValue={lease.adjustment_index ?? 'IGPM'}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '16px', marginTop: '16px' }}>
                    ...
                    <button type="submit" disabled={isLoading} style={{ padding: '14px 28px', borderRadius: '12px', border: 'none', background: 'var(--success-bg)', color: 'var(--success-color)', fontWeight: 'bold', cursor: 'pointer', opacity: isLoading ? 0.3 : 1 }}>
                      {isLoading ? 'Criando cópia...' : 'Criar Contrato Duplicado'}
                    </button>
                  </div>
                </form>
              </>
