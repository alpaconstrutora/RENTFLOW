...
import { Edit, X, Camera, Search, Loader2 } from 'lucide-react'
...
export default function ImovelEditBtn({ userId, property }: Props) {
  ...
  const [photoUrl, setPhotoUrl]         = useState<string | null>(property.photo_url)
  const [photoLoading, setPhotoLoading] = useState(false)
  const fileRef = useRef<HTMLInputElement>(null)

  // Endereço
  const [cep, setCep]           = useState(property.zip_code     ?? '')
  ...
  async function handleSubmit(formData: FormData) {
    ...
    const error = await updatePropertyAction(formData)
    setIsLoading(false)
      ...
      {isOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(5,5,8,0.85)', backdropFilter: 'blur(20px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', textAlign: 'left' }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '540px', background: 'rgba(25,28,38,0.97)', padding: '36px', position: 'relative', borderRadius: '24px', boxShadow: '0 30px 60px rgba(0,0,0,1)', border: '1px solid rgba(255,255,255,0.1)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <h2 style={{ fontSize: '24px', fontFamily: 'var(--font-heading)', marginBottom: '4px', color: 'white' }}>Editar Imóvel</h2>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                  ...
                  <div>
                    ...
                    <input name="purchase_value" type="number" step="0.01" defaultValue={property.purchase_value ?? ''} placeholder="Opcional" style={inp} />
                  </div>
                </div>
              </div>
