...
export default function TenantEditBtn({ userId, tenant }: Props) {
  ...
  const initialType = (tenant.type === 'company' ? 'company' : 'individual') as 'individual' | 'company'
  const [tenantType, setTenantType] = useState<'individual' | 'company'>(initialType)
  const [document, setDocument]   = useState(tenant.document ?? '')
  const [docError, setDocError]   = useState('')
  const [phone, setPhone]         = useState(tenant.phone ?? '')
  const [email, setEmail]         = useState(tenant.email ?? '')
  const [companyName, setCompanyName] = useState(initialType === 'company' ? (tenant.name ?? '') : '')
  const [streetNumber, setStreetNumber] = useState(tenant.street_number ?? '')
  const [guarantorDoc, setGuarantorDoc] = useState(tenant.guarantor_document ?? '')
  ...
  // Photo
  const [photoPreview, setPhotoPreview] = useState<string | null>(tenant.photo_url ?? null)
  const [photoUrl, setPhotoUrl]         = useState<string | null>(tenant.photo_url ?? null)
  ...
  const [uf, setUf]             = useState(tenant.state ?? '')
  ...
  function handleDocumentChange(raw: string) {
    ...
    setDocument(masked); setDocError('')
  }
  ...
  async function handleSubmit(formData: FormData) {
    ...
    const error = await updateTenantAction(formData)
    setIsLoading(false)
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div>
                    ...
                    {tenantType === 'company' ? (
                      ...
                      <input key="name-individual" name="name" defaultValue={tenant.name} required style={inp} />
                    )}
                  </div>
                  ...
                  {tenantType === 'individual' && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                      ...
                      <div>
                        ...
                        <input name="nationality" defaultValue={tenant.nationality ?? ''} placeholder="Brasileira" style={inp} />
                      </div>
                      <div>
                        <label style={lbl}>Estado Civil</label>
                        <select name="marital_status" defaultValue={tenant.marital_status ?? ''} style={{ ...inp, appearance: 'auto' }}>
