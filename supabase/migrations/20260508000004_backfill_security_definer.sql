-- Corrige backfill_lease_history: SECURITY INVOKER bloqueava RLS
-- no SELECT de leases quando auth.uid() era NULL em Server Actions.
-- Solução: SECURITY DEFINER com validação explícita de ownership.
...
DECLARE v_count int;
BEGIN
  -- Valida que o contrato pertence ao usuário autenticado
  IF NOT EXISTS (
